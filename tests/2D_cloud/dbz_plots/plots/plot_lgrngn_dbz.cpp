...
int main(int ac, char** av)
{
  ...
  std::string dir = string(av[1]) + "/dbz_plots/plots/";
...

  {
    ...
    for (int at = 0; at < n["t"]; ++at)
    {
      ...
      {
        for (auto &plt : std::set<std::string>({"Z", "rl", "rr", "nc", "nr", "ef", "na", "rd", "sd_conc", "th", "rv"})) 
        {
          Gnuplot gp;
  	    ...
  	    // rain water content
  	    //
  	    auto tmp = h5load(h5, "rw_rng002_mom6", at * n["outfreq"]);
  	    gp << "set logscale cb\n";
  	    gp << "set title '6th moment [m6/kg]'\n";
  	    //gp << "set cbrange [1e-2:1]\n";
  	    plot(gp, tmp);
  	    gp << "unset logscale cb\n";
          }
